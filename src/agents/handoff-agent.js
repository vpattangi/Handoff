...
function validateResponse(response, retrievedData) {
  const flags = [];
  const responseText = response.toLowerCase();
  const dataText = JSON.stringify(retrievedData).toLowerCase();

  const keyTerms = ['section', 'circular', 'process', 'system', 'contact'];
  keyTerms.forEach(term => {
    const regex = new RegExp(term + '\\w*', 'gi');
    const matches = responseText.match(regex);
    if (matches) {
      matches.forEach(match => {
        if (!dataText.includes(match.toLowerCase())) {
          flags.push(`Unverified reference: ${match}`);
        }
      });
    }
  });
...
Respond ONLY in valid JSON with these exact keys: criticalSystems (array of strings), undocumentedProcesses (array of strings), keyRelationships (array of strings), urgentRisks (array of strings), exitInterviewQuestions (array of strings). No extra text before or after the JSON.`
  ...
  try {
    ...
    const validated = validateResponse(JSON.stringify(parsed), data);
    if (!validated.verified) {
      console.log('[Handoff] Validation flags:', validated.flags);
    }
    return parsed;
  } catch(e) {
